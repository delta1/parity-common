...
pub fn test_iter(db: &dyn KeyValueDB) -> io::Result<()> {
	...
	let contents: Vec<_> = db.iter(0).collect();
...
pub fn test_iter_with_prefix(db: &dyn KeyValueDB) -> io::Result<()> {
	...
	// empty prefix
	let contents: Vec<_> = db.iter_with_prefix(0, b"").collect();
	...
	// prefix a
	let contents: Vec<_> = db.iter_with_prefix(0, b"a").collect();
	...
	// prefix abc
	let contents: Vec<_> = db.iter_with_prefix(0, b"abc").collect();
	assert_eq!(contents.len(), 2);
	assert_eq!(&*contents[0].0, key3);
	assert_eq!(&*contents[1].0, key4);

	// prefix abcde
	let contents: Vec<_> = db.iter_with_prefix(0, b"abcde").collect();
	assert_eq!(contents.len(), 0);

	// prefix 0
	let contents: Vec<_> = db.iter_with_prefix(0, b"0").collect();
